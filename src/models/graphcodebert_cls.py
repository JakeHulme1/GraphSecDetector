...
class GCBertClassifier(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        overrides = {}
        hdp = getattr(cfg, "hidden_dropout_prob", None)
        cdp = getattr(cfg, "classifier_dropout", None)
        if hdp is not None: overrides["hidden_dropout_prob"] = hdp
        if cdp is not None: overrides["classifier_dropout"] = cdp
