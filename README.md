...
The `scripts/train_all.sh` script automates fine-tuning across all seven vulnerability datasets using GraphCodeBERT.  
It performs a grid search over learning rate (LR) and weight decay (WD), builds the Docker image, and launches runs via Hare.
Note. This training script was built for the University of Bath Hex Cloud.

### Usage
```bash
bash scripts/train_all.sh [GPU_DEVICE]
```

`GPU_DEVICE` is optional (default: 2).

Logs, metrics, and checkpoints are saved under output-graphsec/

### Changing Hyperparameters

The hyperparameter sweeps are controlled by the following lines in `train_all.sh`:
